...
# TODO extend the "value exist" check with fuel & vendor
updatePriceInto <- function(table) {
  ...
  function(vendor, date, fuel, price, origin) {
    vendorNow <- vendor # warrning, has to provide new var, table$vendor == vendor would not work in filter
    fuelNow <- fuel
    ...
    if (date %in% tableNow$date) {
      ...
      if (price %in% currentPrices) {
        print(paste0("[Warrning] Zaznam jiz existuje: ", date," > ", fuel, " : ", price, " /", origin, "/"))
        NULL
      } else {
        # TODO d-1 zmena....
...
knitr::kable(tonoPL_withVAT %<>% `colnames<-`(c("Fuel","P_w_VAT")))
...
# rbind dataTable
if (priceListJET[1] == "JETA-1" && priceListJET[3] == "bezSPD(10.95)") {
